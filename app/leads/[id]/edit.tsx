      ...
      <View className="flex-1 justify-end bg-black/40">
        <View className="bg-white rounded-t-3xl px-5 pt-4 pb-10 max-h-[70%]">
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            {options.map((o: any) => {
              const val = typeof o === 'string' ? o : o.value ?? String(o.id);
              const label = renderLabel ? renderLabel(o) : (typeof o === 'string' ? o : o.label);
              const sel = selected === val;
              return (
                <TouchableOpacity key={val} onPress={() => { onSelect(val); onClose(); }}
...
function StyledInput({ value, onChangeText, placeholder, keyboardType, multiline, error, ...rest }: any) {
  return (
    <TextInput
      ...
      className={`bg-white border-2 rounded-xl px-4 py-3 text-sm text-gray-800 font-medium ${error ? 'border-red-300' : 'border-gray-200'} ${multiline ? 'min-h-[90px]' : 'h-12'}`}
      {...rest}
    />
  );
}
...
function SelectBtn({ value, placeholder, onPress, error }: any) {
  return (
    <TouchableOpacity onPress={onPress}
      className={`flex-row items-center justify-between bg-white border-2 rounded-xl px-4 h-12 ${error ? 'border-red-300' : 'border-gray-200'}`}>
...
function PriorityRadio({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  const styles: Record<string, string> = {
    HIGH: 'bg-red-100 border-red-400', MEDIUM: 'bg-yellow-100 border-yellow-400', LOW: 'bg-green-100 border-green-400',
  };
  const textStyles: Record<string, string> = { HIGH: 'text-red-700', MEDIUM: 'text-yellow-700', LOW: 'text-green-700' };
  return (
      ...
      {PRIORITY_OPTIONS.map(p => {
        ...
        return (
          <TouchableOpacity key={p} onPress={() => onChange(p)}
            ...
            <Text className={`text-xs font-bold ${sel ? textStyles[p] : 'text-gray-500'}`}>{p}</Text>
          </TouchableOpacity>
        );
      })}
    </View>
...
export default function EditLeadScreen() {
    ...
    const load = async () => {
      ...
      try {
        const [leadRes, usersRes] = await Promise.all([
          api.get(`/leads/${id}/`),
          api.get('/leads/available-users/').catch(() => null),
        ]);

        const lead = leadRes.data;
        let assignedToValue = '';
        if (lead.assigned_to) {
          if (typeof lead.assigned_to === 'object') assignedToValue = String(lead.assigned_to.id ?? '');
          else assignedToValue = String(lead.assigned_to);
        }

        setFormData({
          name: lead.name ?? '', phone: lead.phone ?? '',
          email: lead.email ?? '', location: lead.location ?? '',
          priority: lead.priority ?? 'MEDIUM', status: lead.status ?? 'ENQUIRY',
          program: lead.program ?? '', source: lead.source ?? '',
          customSource: lead.custom_source ?? '', remarks: lead.remarks ?? '',
        ...
        if (usersRes) {
          ...
          if (role === 'ADM_MANAGER') users = users.filter((u: any) => ['ADM_MANAGER','ADM_EXEC','FOE'].includes(u.role));
          else if (role === 'FOE' || role === 'ADM_EXEC') users = users.filter((u: any) => u.id === user?.id);
          setUsers(users);
        }
      } catch {
  ...
  const validate = () => {
    ...
    if (formData.email && !/\S+@\S+\.\S+/.test(formData.email)) e.email = 'Email is invalid';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.put(`/leads/${id}/`, {
        ...
        program: formData.program || null, source: formData.source,
        custom_source: formData.source === 'OTHER' ? formData.customSource.trim() : '',
        remarks: formData.remarks?.trim() || '',
      });

      // Step 2: Handle assignment
      const assignedToValue = formData.assignedTo ? parseInt(formData.assignedTo) : null;
      if (assignedToValue) {
        await api.post('/leads/assign/', { lead_id: parseInt(id), assigned_to_id: assignedToValue });
      } else {
        await api.post('/leads/unassign/', { lead_id: parseInt(id), unassign_type: 'PRIMARY' });
      }
      ...
      setApiError(data?.detail || data?.message || 'Failed to update lead');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <View className="flex-1 bg-slate-50">
      ...
      <View className="bg-indigo-50 px-4 pt-14 pb-4 flex-row items-center gap-3">
        ...
        <View>
          <Text className="text-lg font-extrabold text-indigo-600">Edit Lead</Text>
          <Text className="text-xs text-gray-500">Update the lead information below</Text>
        </View>
      </View>
      ...
      <ScrollView className="flex-1 px-4 pt-4" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
        ...
        {submitted && (
          <View className="flex-row items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-4 py-3 mb-4">
            ...
            <Text className="text-green-700 font-semibold text-sm">Lead updated successfully! Redirecting…</Text>
          </View>
        )}
        ...
        <View className="bg-white rounded-2xl border border-gray-100 p-4 mb-4">
          ...
          <Field label="Assign To" error={errors.assignedTo}>
            <SelectBtn value={selectedUserLabel} placeholder="Select a staff member" onPress={() => setPicker('assignedTo')} error={errors.assignedTo} />
          </Field>
        </View>
        ...
        <View className="bg-white rounded-2xl border border-gray-100 p-4 mb-4">
          ...
          <Field label="Remarks / Notes">
            <StyledInput value={formData.remarks} onChangeText={set('remarks')} placeholder="Add any additional notes…" multiline />
          </Field>
        </View>
        ...
        <View className="flex-row items-start gap-2 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 mb-6">
          ...
          <Text className="text-xs text-blue-700 flex-1">
            Assignment changes will be reflected immediately after updating.
          </Text>
        </View>
        ...
        <View className="flex-row gap-3">
          <TouchableOpacity onPress={handleBack} disabled={submitting}
            className="flex-1 py-3.5 border-2 border-gray-200 rounded-xl items-center">
            <Text className="text-sm font-semibold text-gray-700">Cancel</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={handleSubmit} disabled={submitting || submitted}
            ...
            <Text className="text-sm font-bold text-white">{submitting ? 'Updating…' : 'Update Lead'}</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>

      {/* Picker modals */}
      <PickerModal visible={picker === 'program'} title="Select Program" options={PROGRAM_OPTIONS}
        selected={formData.program} onSelect={set('program')} onClose={() => setPicker(null)} />
      <PickerModal visible={picker === 'status'} title="Select Status" options={STATUS_OPTIONS}
        selected={formData.status} onSelect={set('status')} onClose={() => setPicker(null)} />
      <PickerModal visible={picker === 'source'} title="Select Source" options={SOURCE_OPTIONS}
        selected={formData.source} onSelect={set('source')} onClose={() => setPicker(null)} />
      <PickerModal visible={picker === 'assignedTo'} title="Assign To" options={availableUsers}
        selected={formData.assignedTo}
        renderLabel={(u: any) => `${u.first_name} ${u.last_name} — ${ROLE_DISPLAY[u.role] ?? u.role}`}
        onSelect={(val: string) => set('assignedTo')(val)} onClose={() => setPicker(null)} />
    </View>
  );
}